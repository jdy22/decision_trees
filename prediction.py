...
def predict_label(trained_tree: Node, test_db):
    ...
    for index, instance in enumerate(test_db):
        current_node = trained_tree
        # traversing between left and right node based on current_node's split threshold until leaf node is reached
        while not current_node.is_leaf:
            if instance[current_node.split.attribute_index] <= current_node.split.threshold:
...
if __name__ == "__main__":
    ...
    print(predict_label(root_node, np.array([[-55, -68, -78, -59, -65, -60, -80]])))
